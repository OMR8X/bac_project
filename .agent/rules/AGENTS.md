...
### `lib/presentation/` - Presentation Layer <!-- Presentation layer for features -->
Features are typically organized into `lib/presentation/<feature>/`. Each feature directory follows a consistent structure:
- **state/**: Contains BLoC/Cubit for state management <!-- Business logic and state management -->
- **view/**: Contains the main screens/pages <!-- Main UI screens -->
- **widgets/**: Contains reusable widgets specific to the feature <!-- Feature-specific components -->

### `lib/core/` - Shared Layer <!-- Core shared resources and utilities -->
Contains logical and UI components shared across the application:

#### `resources/` <!-- shared design system and constants -->
Core resources that define the visual language and shared constants:
- **styles/**: Predefined resources for styling widgets. Use these instead of hardcoded styles.
- **themes/**: Application theme definitions using Material 3 color patterns and surfaces.
- **errors/**: Standardized error handling classes.

#### Other Core Directories
- **services/**: Infrastructure services (API, Storage, Navigation).
- **widgets/**: Common UI components used across multiple features.
